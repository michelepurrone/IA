...
public:
  UnloadAction()
  : plansys2::ActionExecutorClient("unload_box_from_carrier", 1s)
  {
    progress_ = 0.0;
  }
...
private:
  void do_work()
  {
    if (progress_ < 1.0) {
      progress_ += 0.1;
      send_feedback(progress_, "Unload running");
    } else {
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "unload_box_from_carrier"));
