...
class hRGCHeuristic(_RelaxationHeuristic):

    """
    L'euristica RGC stima il numero di goal che non sono ancora stati raggiunti nello stato corrente.
    Essa non considera le distanze o i costi per raggiungere l’obiettivo, preferendo fornire una semplice stima basata sul conteggio dei goal rimanenti. 
    Può essere particolarmente utile in tutti quei problemi che tralasciano i costi o le distanze specifiche, ma che pongono come obiettivo principale solo il raggiungimento dei goal.
    """
...
class hHammingHeuristic(_RelaxationHeuristic):
    ...
    """
    L'euristica Hamming stima il numero di differenze tra lo stato corrente e lo stato obiettivo calcolando la distanza di Hamming: si tratta del conteggio delle posizioni in cui gli elementi corrispondenti in due stati sono diversi.
    """
    ...
    def __call__(self, node):
        ...
        h_value = sum(current_fact != goal_fact for current_fact, goal_fact in zip(current_state, goal_state)) #Il valore restituito è la somma dei confronti tra gli elementi di current_state e goal_stat
